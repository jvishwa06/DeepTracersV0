...
export default function Page() {
  ...
  const [confidenceScore, setConfidenceScore] = useState<number | null>(null); // New state for confidence score
  const [isChatOpen, setIsChatOpen] = useState(false);
  const [chatMessages, setChatMessages] = useState<Array<{ type: 'user' | 'bot', content: string }>>([]);
  const [userInput, setUserInput] = useState('');

  // Function to call the LLM API
  const callLLM = async (userInput: string) => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to fetch response from LLM: ${errorDetails}`);
      }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setIsDeepfakeDetected(data.prediction === 'fake');
        setConfidenceScore(data.confidence); // Set confidence score

        if (data.prediction === 'fake') {
          toast.error(`Deepfake detected with confidence ${(data.confidence * 100).toFixed(2)}%.`, {
            position: "top-center",
            autoClose: 5000,
          ...
          toast.success(`No deepfake detected with confidence ${(data.confidence * 100).toFixed(2)}%.`, {
            position: "top-center",
            autoClose: 3000,
            hideProgressBar: false,
            closeOnClick: true,
  ...
  const handleChatSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const botResponse = await callLLM(userInput);
  ...
  return (
    <div className="container mx-auto px-4 py-8 relative">
      <h3 className="text-4xl font-bold mb-8 text-center">DeepTracerV0 - Advanced Deepfake Detection Platform </h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        {isDeepfakeDetected !== null && confidenceScore !== null && (
    <Alert className={`mt-8 ${isDeepfakeDetected ? 'bg-red-100 border-red-400 text-red-700' : 'bg-green-100 border-green-400 text-green-700'}`}>
      ...
      <AlertDescription>
        {isDeepfakeDetected
          ? `Our analysis suggests that this image may be a deepfake with a confidence of ${(confidenceScore * 100).toFixed(2)}%. Please review the results carefully and consider reporting suspicious content.`
          : `Our analysis suggests that this image is likely not a deepfake with a confidence of ${(confidenceScore * 100).toFixed(2)}%. However, always exercise caution and critical thinking when evaluating online content.`}
      </AlertDescription>

      {/* Conditionally render the Report button only when there are search results */}
      {isDeepfakeDetected && searchResult && searchResult.length > 0 && (
        <div className="flex justify-end mt-4">
          <Button variant="destructive" onClick={() => handleReport(1)}>
            Report
          </Button>
        </div>
      )}
      ...
      <Tabs defaultValue="about" className="mt-8">
        ...
        <TabsContent value="about">
          <Card>
            <CardHeader>
              <CardTitle>About DeepTracer</CardTitle>
            </CardHeader>
            <CardContent>
              <p>DeepTracer is a cutting-edge tool designed to help you identify potential deepfakes and manipulated images. Our advanced algorithms search across multiple platforms to find similar images and analyze their authenticity.</p>
            </CardContent>
          </Card>
        </TabsContent>
      ...
      {isChatOpen && (
        <div className="fixed bottom-20 right-4 w-80 h-96 bg-white border border-gray-200 rounded-lg shadow-lg flex flex-col">
          <div className="flex justify-between items-center p-4 border-b">
            <h3 className="font-semibold">DeepTracer Assistant</h3>
