...
const PostForm = ({ formData, handleChange, onSubmit }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const name = data.prediction; // Assuming 'prediction' is the correct field
      console.log("Response from server:", data);

      if (name === "fake") {
        setShowPopup(true);
      } else if (name === "real" || "no_face_detected") {
        setShowSuccess(true);
        onSubmit({ mediaPreview, title, description });
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50">
          <div className="rounded-lg p-8" style={{ backgroundColor: "black" }}>
            <p className="text-white-800 mb-4">
              ...
              <br /> Your post was successfully published!
            </p>
            <button
